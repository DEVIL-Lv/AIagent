...
const Dashboard: React.FC = () => {
  ...
  const handleSendMessage = async () => {
      ...
      if (!selectedCustomerId) {
          ...
          try {
              const res = await customerApi.chatGlobal(msg, selectedModel);
              ...
              setGlobalChatHistory(prev => [...prev, { 
                  role: 'ai', 
                  content: "(系统错误) 连接 AI 失败，请检查网络或配置。", 
                  timestamp: new Date().toISOString() 
              }]);
          }
          return;
      ...
      try {
          const res = await customerApi.agentChat(selectedCustomerId, msg, chatHistory, selectedModel);
          setChatHistory(prev => [...prev, { role: 'ai', content: res.data.response, timestamp: new Date().toISOString() }]);
      } catch (error) {
          message.error("发送失败");
          setChatHistory(prev => [...prev, { role: 'ai', content: "思考超时，请重试。", timestamp: new Date().toISOString() }]);
      }
  };
  ...
  const handleQuickAsk = async (question: string) => {
      ...
      try {
          const res = await customerApi.agentChat(selectedCustomerId, question, chatHistory, selectedModel);
          setChatHistory(prev => [...prev, { role: 'ai', content: res.data.response, timestamp: new Date().toISOString() }]);
      } catch (error) {
          message.error("请求失败");
          setChatHistory(prev => [...prev, { role: 'ai', content: "助手暂时无法响应，请稍后重试。", timestamp: new Date().toISOString() }]);
      }
  };
                          ...
                          currentHistory.map((msg, idx) => (
                              <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-fade-in`}>
                                  <div className={`flex gap-2 max-w-[90%] ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                                      ...
                                      />
                                      <div>
                                          <div className={`px-3 py-2 rounded-xl text-sm whitespace-pre-wrap leading-relaxed ${
                                              msg.role === 'user' 
